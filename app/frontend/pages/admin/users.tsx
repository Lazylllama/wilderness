...
export default function AdminUsers({
    ...
    function toggleAdmin(user: AdminUser) {
		router.patch(`/admin/users/${user.id}`, {field: "admin"}, {preserveScroll: true});
	}

    return (
        <AdminShell title="Users" subtitle="Sorted by most recent RSVP" flashNotice={flash_notice}>
			<div className="flex flex-col gap-4">
				<form onSubmit={runSearch} className="flex flex-row gap-2">
					...
					/>
					<Button type="submit" variant="secondary" className="h-11" aria-label="Search">
						<Search size={18} strokeWidth={3} />
					</Button>
				</form>

				<AdminTable
					headers={["User", "Slack ID", "Verification", "RSVP", "Projects", "Role", ""]}
						...
						<tr key={user.id} className="border-t border-border">
							<td className="px-4 py-3">
								<div className="flex flex-col">
									...
									<span className="font-serif text-sm text-foreground/50">{user.email}</span>
								</div>
							</td>
