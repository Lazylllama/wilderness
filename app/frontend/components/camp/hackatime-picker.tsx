...
export function HackatimePicker({
	...
	<>
		<div className="flex flex-col gap-2">
			<div className="flex items-center justify-between">
				...
				<button
					...
					onClick={onSync}
					disabled={syncing}
					className="flex cursor-pointer items-center gap-1 font-serif text-xs italic text-foreground/50 transition-colors hover:text-foreground disabled:opacity-50"
				>
		...
		{projects.length === 0 ? (
			<p className="rounded-lg border border-dashed border-border p-4 text-center font-serif text-sm text-foreground/50">
				no hackatime projects yet. link your hackatime from the ranger post, and
				hit resync. if not yet, you can{" "}
				<a href="https://hackatime.hackclub.com/setup">setup hackatime</a>
			</p>
		) : (
			<div>
				{projects.map((project) => {
					...
					return (
						...
						>
							<span>{isSelected && <Check size={14} strokeWidth={4} />}</span>
							<span className="flex min-w-0">
								...
								<span className="font-serif text-xs text-foreground/50">
									{takenByOther
										? `claimed by ${project.claimed_by}`
										: `last heartbeat ${relativeTime(project.last_heartbeat)}`}
								</span>
							</span>
